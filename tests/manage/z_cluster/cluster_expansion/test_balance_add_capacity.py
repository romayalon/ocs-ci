...
class TestRebalaceAddCapacity(ManageTest):
    ...
    def test_rebalance_add_capacity(self, workload_storageutilization_rbd):
        ...
        logger.info("Checking osds utilization")
        ...
        check_until_osd_ratio_start_decrease_or_equal(osds_before_expand, digit_point=2, ratio_stable=1,
                                                      exp_time=int(300 + 240 * num_of_osd_factor))
        logger.info("Adding capacity to the cluster")
        ...
        """Waiting for old osd sum of utilization to go down and new osd to go up"""
        check_rebalance_occur_after_expand(osds_before_expand, exp_time=int(300 + 1200 * num_of_osd_factor))
        logger.info("Deleting pod and pvc")
        ocp = OCP()
        ...
        logger.info("Waiting for one minute to check ceph health again")
        sleep(60)
        ceph_health_check(
